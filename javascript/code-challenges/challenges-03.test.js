...
const typeNum = (arr) => {
  // Solution code here...
  let NumArr = arr.filter((index) => {
    return (typeof (index) === typeof (1));
  });
  return NumArr;
};
...
const containsAnd = (arr) => {
  let ArrContainsAnd = arr.filter((and) => {
    return (and.includes('and'));
  }
  );
  return ArrContainsAnd;
  // Solution code here...
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let same = arr.filter(y => !forbiddenValues.includes(y));
  return same;
