...
const updateNumbers = (obj) => {
  // Solution code here...
  return Object.entries(obj).map((elemnt)=>elemnt.join(': '));
...
const getHouses = (arr) => {
  let houses = [];
  arr.foreach((elemnt)=>{houses.push(elemnt.houses);});
  // Solution code here...
  return houses;
};
...
let final = [];
arr.forEach(elemnt => {
  if (elemnt.name === character) {
    let index = Object.keys(elemnt).indexOf('children');
    if(index >= 0){
      final = Object.values(elemnt)[index];
    }
  }
});
return final.length ? true : false;
};
